...
classdef UnitExtracel < dj.Imported
	...
	methods(Access=protected)
        ...
        function makeTuples(self, key)
            ...
            for iUnits = 1:1:size(obj.eventSeriesHash.value,2)
                key.unit_id = size(fetch(DATA.UnitExtracel),1) + 1;
                key.unit_num = iUnits;
                key.unit_hemisphere =  fetch1(DATA.ExtracelProbe & key,'recording_hemisphere');
                key.unit_brain_area =  fetch1(DATA.ExtracelProbe & key,'recording_brain_area');
           ...
           sprintf('Populated %d units recorded from animal %d  on %s', iUnits, key.animal_id, fetch1(DATA.Session & key,'session_date'));
